...
class ChickenProductionDashboard extends StatelessWidget {
  const ChickenProductionDashboard({super.key});

  @override
  Widget build(BuildContext context) {
    return BlocListener<ChickenProductionCubit, ChickenProductionState>(
      listener: (context, state) {
        log('ChickenProduction state: $state');
        if (state is ChickenProductionLoaded) {
          log('ChickenProduction data loaded: ${state.models}');
        }
        if (state is ChickenProductionError) {
          log('ChickenProduction error: ${state.message}');
        }
      },
      child: Column(
    ...
    return BlocBuilder<CageCubit, CageState>(
      builder: (context, cageState) {
        if (cageState is CageLoaded) {
          final selectedCageId = context.select((ChickenProductionCubit cubit) => cubit.selectedCageId);
          ...
          return DropdownButton2<String>(
            ...
            onChanged: (value) {
              if (value != null) {
                context.read<ChickenProductionCubit>().updateSelectedCage(value);
  ...
  Widget _buildLineChart(BuildContext context) {
    return SizedBox(
      height: 250,
      child: BlocBuilder<ChickenProductionCubit, ChickenProductionState>(
        builder: (_, state) {
  ...
  Widget _buildSummaryGrid() {
    return BlocBuilder<ChickenProductionCubit, ChickenProductionState>(
      builder: (_, state) {
        if (state is ChickenProductionLoaded) {
          final summary = [
